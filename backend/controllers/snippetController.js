...
const { sendForkNotificationEmail } = require("../utils/mailer");
...
exports.forkSnippet = async (req, res) => {
    try {
        ...
        if (original.userId !== req.user) {
            const owner = await User.findByPk(original.userId);
            const forker = await User.findByPk(req.user);
            if (owner && owner.email) {
                sendForkNotificationEmail(
                    owner.email,
                    owner.username,
                    forker?.username || "Someone",
                    original.title || "Untitled Snippet"
                );
            }
        }
